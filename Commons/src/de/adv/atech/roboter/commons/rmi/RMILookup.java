...
public class RMILookup implements Runnable {
	...
	public static synchronized void stop() {
		...
		ControllerManager.debug("RMI lookup: stopping listeners...");

		Object[] lookup = _threadList.toArray();
		for (int i = 0; i < lookup.length; i++) {
			((Thread) lookup[i]).interrupt();
			ControllerManager.debug("Interrupting thread " + lookup[i]);
		}
	 ...
	 * 
	 * @param service
	 *            RMI server - used to match interface requests and to serialize
	 *            back to disciverer
	 * @param serviceName
	 *            Unique name associated with this service. This parameter is
	 *            used to distinguish between multiple intances of the service
	 */
	 ...
	 *            used to distinguish between multiple intances of the service
	 * @param port
	 *            registry port to bind to (if running)
	 */
	...
	public static void bind(Remote service, String serviceName, int port) {
		...
		ControllerManager.debug("RMI lookup: Starting up for " + serviceName);
	...
	public void run() {
		try {
			ControllerManager.debug("RMI lookup: listening....");

			MulticastSocket socket = new MulticastSocket(Discovery
					.getMulticastPort());
			InetAddress address = Discovery.getMulticastAddress();
			ControllerManager.debug("RMI lookup: Multicast address=" + address);
			...
			while (!Thread.currentThread().isInterrupted()) {
				...
				try {
					...
					// ControllerManager.debug(ex.getMessage());
					// Nichts passiert, nichts ausgeben
				...
				ControllerManager.debug("RMI lookup: Recieved messaged " + msg);
				if (msg.startsWith(Discovery.getProtocolHeader())) {
					...
					ControllerManager.debug("RMI lookup: Service name="
							+ serviceName);
					ControllerManager.debug("RMI lookup: Service interface="
							+ interfaceName);
					...
					if (!serviceName.equals(Discovery.ANY)
						...
						ControllerManager.debug(getClass()
					...
					for (int i = 0; !match && i < _serviceInterface.length; i++) {
						match = _serviceInterface[i].getName().equals(
								interfaceName);
					}
					if (match) {
						try {
		...
		ControllerManager.debug("RMI Lookup: listener exiting");
	}
