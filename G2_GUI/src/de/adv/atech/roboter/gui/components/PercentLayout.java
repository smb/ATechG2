...
public class PercentLayout implements LayoutManager2 {
	...
	public Dimension preferredLayoutSize(Container parent) {
		...
		return new Dimension(width + insets.right + insets.left, height
	...
	public void layoutContainer(Container parent) {
		...
		for (int i = 0, c = components.length; i < c; i++) {
			if (components[i].isVisible()) {
				...
				if (constraint == null || constraint == PREFERRED_SIZE) {
					sizes[i] = (HORIZONTAL == orientation ? components[i]
		...
		int currentOffset = (HORIZONTAL == orientation ? insets.left
				: insets.top);
